...
export class AdminDashboardComponent implements OnInit{
  ...
  adminProductDashboard(){
    this.adminService.productDashboardData().subscribe(data =>{
      this.product_databoard_data =data;
      console.log(this.product_databoard_data);
      for(let status in this.product_databoard_data){
        if(this.product_databoard_data[status].status =='public'){
          ++this.publish_product;
        }else if(this.product_databoard_data[status].status =='inactive'){
          ++this.inactive_product;
        }else if(this.product_databoard_data[status].status =='draft'){
          ++this.draft_product;
        }
        ++this.total_product;
      }
